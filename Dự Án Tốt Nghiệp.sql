...
CREATE TABLE Vouchers (
    ...
    DiscountValue DECIMAL(18,2) NOT NULL CHECK (DiscountValue > 0), -- Thêm dấu cách ở đây
...
-- Thống kê tổng doanh thu theo từng tháng/năm cho các đơn hàng đã hoàn tất
SELECT 
    YEAR(OrderDate) AS ReportYear, 
    MONTH(OrderDate) AS ReportMonth, 
    SUM(TotalAmount) AS TotalRevenue, 
    COUNT(BillID) AS TotalOrders
FROM Bills
WHERE OrderStatus = N'Completed' -- Chỉ tính những đơn đã giao và thu tiền thành công
...
SELECT SUM(TotalAmount) AS TotalRevenue FROM Bills WHERE OrderStatus = N'Completed';

-- 2. Đếm tổng số đơn hàng
SELECT COUNT(BillID) AS TotalOrders FROM Bills;
