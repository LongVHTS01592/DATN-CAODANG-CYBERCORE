...
-- ==========================================
-- 2. TẠO CÁC BẢNG ĐỘC LẬP (KHÔNG CÓ KHÓA NGOẠI)
-- ==========================================
CREATE TABLE Roles (
...
CREATE TABLE Vouchers (
    ...
    VoucherCode VARCHAR(50) NOT NULL UNIQUE, -- Cột này dùng làm khóa ngoại liên kết
    Description NVARCHAR(255),
...
-- ==========================================
-- 3. TẠO CÁC BẢNG CẤP 1 (PHỤ THUỘC 1 KHÓA NGOẠI)
-- ==========================================
CREATE TABLE Users (
...
-- ==========================================
-- 4. TẠO CÁC BẢNG CẤP 2 (PHỤ THUỘC NHIỀU KHÓA NGOẠI)
-- ==========================================
...
-- BẢNG BILLS: Được thêm khóa ngoại trực tiếp liên kết chặt chẽ với Vouchers
CREATE TABLE Bills (
    ...
    TotalAmount DECIMAL(18,2) NOT NULL, -- Số tiền thực tế phải thanh toán (sau giảm giá)
    VoucherCode VARCHAR(50) NULL,        -- Cho phép NULL nếu đơn hàng không dùng mã
    OrderStatus NVARCHAR(50) DEFAULT N'Pending',
    Notes NVARCHAR(255),
    CONSTRAINT FK_Bills_Users FOREIGN KEY (UserID) REFERENCES Users(UserID),
    CONSTRAINT FK_Bills_Vouchers FOREIGN KEY (VoucherCode) REFERENCES Vouchers(VoucherCode) -- Khóa ngoại kết nối thực thể
);
...
-- ==========================================
-- 5. TẠO CÁC BẢNG CHI TIẾT (BẢNG TRUNG GIAN)
-- ==========================================
...
CREATE TABLE Payments (
    ...
    Amount DECIMAL(18,2) NOT NULL, -- Số tiền khớp với TotalAmount bên hóa đơn đã trừ voucher
...
-- CHÈN DỮ LIỆU VOUCHER MẪU ĐỂ TEST LUỒNG CHECKOUT
